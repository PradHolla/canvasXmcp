...
class ModelTester:
    ...
    async def run_query(self, query: str, query_id: int, category: str) -> Dict[str, Any]:
        ...
        try:
            config = {
                "configurable": {"thread_id": "memory_test_session"},
                "recursion_limit": 50
            }
            ...
            # Extract token usage and tools from current query turn ONLY
            total_input = 0
            total_output = 0
            tools_used = []
            ...
            # Find where current turn started (last HumanMessage)
            current_turn_start = 0
            for i, msg in enumerate(reversed(result["messages"])):
                if hasattr(msg, '__class__') and msg.__class__.__name__ == 'HumanMessage':
                    current_turn_start = len(result["messages"]) - 1 - i
                    break

            # Extract tool calls from current turn (may be in multiple messages)
            for msg in result["messages"][current_turn_start:]:
...
class TestRunner:
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        output_file = self.results_dir / f"test_results_memory_mt_{self.test_run_id}.json"
    ...
    def _print_summary(self, comparison: Dict[str, Any]):
        ...
        print("\n" + "="*90)
        print(f"\n💾 Detailed results saved: tests/results/test_results_memory_mt_{self.test_run_id}.json")
        print("="*90 + "\n")
