...
\begin{abstract}
Students managing multiple courses face significant challenges tracking assignments, deadlines, and academic content across Learning Management Systems. This project presents a production ready conversational AI assistant for Canvas LMS, leveraging the Model Context Protocol (MCP) and AWS Bedrock to enable natural language interactions with real time student data. The system evolved from an initially proposed multi-agent architecture to a streamlined single agent ReAct framework, demonstrating that careful prompt engineering and tool design can match multi-agent performance on structured domains. Through comprehensive evaluation across 149 test queries, we compare Meta LLaMA 4 Maverick and OpenAI GPT-OSS 120B, revealing counterintuitive findings: GPT-OSS achieved both higher quality (87.9\% pass rate, 8.22/10 average score) and lower cost (19\% cheaper per query), while conversation memory improved GPT-OSS performance (+2.46 points) but degraded LLaMA performance (-0.31 points). The final system features a full-stack architecture with FastAPI backend, React frontend, PostgreSQL persistence, and LangSmith observability, supporting 10+ MCP tools for comprehensive Canvas operations including read access and assignment submission. Systematic failure analysis identified distinct model-specific weaknesses: LLaMA's error handling (46\% of failures exposed raw outputs) versus GPT-OSS's course ID confusion (56\% wrong data retrieval), informing targeted improvement strategies. The system demonstrates practical utility for student workflows while providing valuable insights into LLM based tool orchestration, memory management, and cost quality tradeoffs in educational AI applications.
\end{abstract}
...
Figure~\ref{fig:architecture} illustrates the complete system architecture, showing the interaction flow between the user interface, the ReAct agent (LangGraph), the MCP server, and the Canvas LMS API with AWS Bedrock.
...
\begin{itemize}
    ...
    \item \texttt{GET /api/v1/courses/\{id\}/discussion\_topics}: Access discussion forums and topics
    \item \texttt{GET /api/v1/courses/\{id\}/files}: Retrieve course documents and materials
    \item \texttt{GET /api/v1/courses/\{id\}/modules}: Access course structure and content organization
    \item \texttt{GET /api/v1/users/self/upcoming\_events}: Access calendar events with date filtering
    \item \texttt{POST /api/v1/courses/\{id\}/assignments/\{id\}/submissions}: Submit assignments with file uploads
\end{itemize}
...
\textbf{Tool Definitions:} Thirteen MCP tools implemented, each with typed parameters and structured return values:

\begin{enumerate}
    \item \texttt{get\_courses()}: Returns list of enrolled courses with IDs, names, and enrollment status
    \item \texttt{get\_upcoming\_assignments(days: int, include\_overdue: bool)}: Fetches assignments due within specified window with optional overdue items
    ...
    \item \texttt{get\_announcements(days: int)}: Fetches recent announcements with date filtering
    \item \texttt{get\_discussions(course\_id: str)}: Retrieves discussion topics for a course
    \item \texttt{get\_course\_files(course\_id: str)}: Returns files and documents for a course
    \item \texttt{get\_modules(course\_id: str)}: Accesses course modules/structure with file fallback
    \item \texttt{get\_course\_id\_by\_name(course\_name: str)}: Resolves course names to numeric IDs
    \item \texttt{submit\_assignment(course\_id, assignment\_id, file\_path, comment)}: Submits file to Canvas assignment
\end{enumerate}
