...
\textbf{Model Performance Disparities:} Comparative evaluation revealed distinct performance profiles: GPT-OSS 120B achieved 87.9\% pass rate (8.22/10 average score) versus LLaMA 4 Maverick's 76.5\% pass rate (6.84/10 average). However, LLaMA demonstrated superior cost-efficiency (39\% better quality-per-dollar ratio), suggesting deployment strategy should balance quality requirements against budget constraints.
...
\caption{System Architecture: ReAct Agent with MCP-Based Tool Integration}
\label{fig:architecture}
\end{figure}

Figure~\ref{fig:architecture} illustrates the complete system architecture, showing the interaction flow between the user interface (Chainlit), the ReAct agent (LangGraph), the MCP server, and the Canvas LMS API.
...
Table~\ref{tab:cost_quality} quantifies the cost-efficiency tradeoff.
